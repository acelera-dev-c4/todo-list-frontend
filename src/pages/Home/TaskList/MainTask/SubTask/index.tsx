    ...
    <>
      {subTasks
        ...
        .map((subTask: ISubTask, index) => (
          <div key={`${task.id}-${subTask.id || index}`} className="bg-white rounded">
            <div className="flex items-center justify-between p-3 shadow my-2">
              ...
              {editingSubTaskId === subTask.id ? (
                ...
                <>
                  ...
                  />
                  <PersonAddRounded
                    className="bg-green-500 text-white p-1 rounded ml-2 hover:bg-green-600 cursor-pointer w-6 h-6"
                    fontSize="medium"
                    onClick={() => {
                        if(openSearch === index) {
                          setOpenSearch(null)
